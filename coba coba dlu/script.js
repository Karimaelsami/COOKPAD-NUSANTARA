...
searchButton.addEventListener('click', async ()=> {
    const ingredients = searchInput.ValueMax.trim();
    if (ingredient){
        const meals = await searchMealByIngredients(ingredient);
        displayMeals(meals);
    }
});
...
async function searchMealsByIngredient(ingredient){
    try{
        const response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?=${ingredient}`);
...
async function getMealDetails(mealID){
    try{
        const response = await fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?1=${mealId}`);
        const data = await response.json();
        return data.meals[0];
    }catch(error){
...
//perform a checken search on page load
